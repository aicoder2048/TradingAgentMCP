...
### 必需输出格式
1. **时间基准声明**: 明确声明分析基准时间
2. **推荐策略对比表**: 包含至少3个风险级别的建议
3. **专业订单格式**: JP Morgan风格的交易指令
4. **风险管理剧本**: 开仓、持仓、平仓的具体触发条件
5. **分配避免协议**: Delta漂移和支撑跌破的应对措施
...
请严格按照上述序列执行所有工具，重点关注**收入生成**而非股票获取，确保所有推荐策略的年化收益≥{target_apy_pct}%且分配概率<30%。
