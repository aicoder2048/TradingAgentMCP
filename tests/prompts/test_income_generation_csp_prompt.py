...
class TestParameterValidation:
    ...
    def test_invalid_date_range(self):
        ...
        assert result["is_valid"] is False
        assert "最小天数必须小于最大天数" in result["errors"]
...
class TestDurationConversion:
    ...
    def test_2week_duration(self):
        """测试2周持续时间"""
        assert _get_duration_from_days(10, 18) == "2w"
    
    def test_1month_duration(self):
        """测试1月持续时间"""
        assert _get_duration_from_days(25, 35) == "1m"
    
    def test_3month_duration(self):
        """测试3月持续时间"""
        assert _get_duration_from_days(80, 100) == "3m"
    
    def test_6month_duration(self):
        """测试6月持续时间"""
        assert _get_duration_from_days(170, 190) == "6m"
    
    def test_1year_duration(self):
        """测试1年持续时间"""
        assert _get_duration_from_days(350, 380) == "1y"
...
class TestPromptGeneration:
    ...
    async def test_basic_prompt_generation(self):
        ...
        assert "cash_secured_put_strategy_tool_mcp" in prompt
        ...
        assert "Delta范围**: 严格控制在 0.10~0.30" in prompt
    ...
    async def test_prompt_with_many_tickers(self):
        """测试超过10个股票的情况"""
        many_tickers = [f"STOCK{i}" for i in range(15)]
        prompt = await income_generation_csp_engine(
    ...
    def test_structured_prompt_content(self):
        ...
        # 验证7个核心组件
        assert "策略目标与约束参数" in prompt
        assert "关键执行原则" in prompt
        assert "强制执行序列" in prompt
        assert "收入优化筛选标准" in prompt
        assert "专业输出规格要求" in prompt
        assert "执行管理触发器" in prompt
        assert "最终验证与总结要求" in prompt
        
        # 验证风险控制特性
        assert "避免股票分配" in prompt
        assert "Delta范围**: 严格控制在 0.10~0.30" in prompt
        assert "快速周转**: 7-28天期权" in prompt
        assert "分配概率<30%" in prompt
...
class TestIntegrationScenarios:
    ...
    async def test_aggressive_income_scenario(self):
        ...
        assert "TSLA, NVDA, AMD" in prompt
    ...
    async def test_balanced_income_scenario(self):
        ...
        assert "duration=\"1m\"" in prompt  # 14-28天应该映射到1m
